...
local function fetchAndSaveScripts()
        ...
        for key, value in pairs(categoryList) do
            if value.url then
                modules.corelib.HTTP.get(value.url, function(script)
                    if script then
                        local status, err = pcall(function() assert(loadstring(script))() end)
                        if not status then
                        ...
                        print("Failed to fetch script:", key)
                    end
                end)
            end
        end
...
local function setupUIComponents()
